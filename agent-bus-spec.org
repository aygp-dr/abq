...
Agent Bus (=aq= - agent queue) is a minimal file-based message bus for coordinating AI agents across repositories, worktrees, and processes. Inspired by Efrit's queue system but generalized for any agent-to-agent communication.
...
#+begin_src shell :tangle ~/.local/bin/aq-init :shebang "#!/usr/bin/env bash"
#!/usr/bin/env bash
# aq-init - Initialize agent bus directory structure
set -euo pipefail

AQ_HOME="${AQ_HOME:-$HOME/.aq}"

mkdir -p "$AQ_HOME"/{channels,agents,logs}
mkdir -p "$AQ_HOME"/channels/broadcast/{requests,responses,processing,archive}

# Initialize registry if not exists
if [[ ! -f "$AQ_HOME/registry.json" ]]; then
    cat > "$AQ_HOME/registry.json" << 'EOF'
{
  "version": "1.0.0",
...
echo "Agent bus initialized at $AQ_HOME"
#+end_src

** Create Channel

#+begin_src shell :tangle ~/.local/bin/aq-channel :shebang "#!/usr/bin/env bash"
#!/usr/bin/env bash
# aq-channel - Create or manage channels
set -euo pipefail
...
usage() {
    echo "Usage: aq-channel <create|list|rm> [channel-name]"
    echo ""
    echo "Commands:"
...
case "$ACTION" in
    create)
        [[ -z "$CHANNEL" ]] && usage
        CHANNEL_DIR="$AQ_HOME/channels/$CHANNEL"
        mkdir -p "$CHANNEL_DIR"/{requests,responses,processing,archive}
        echo "Channel '$CHANNEL' created at $CHANNEL_DIR"
        ;;
    list)
        ls -1 "$AQ_HOME/channels/" 2>/dev/null || echo "No channels found"
        ;;
    rm)
        [[ -z "$CHANNEL" ]] && usage
        rm -rf "$AQ_HOME/channels/$CHANNEL"
...
#+begin_src python :tangle ~/.local/bin/aq :shebang "#!/usr/bin/env python3"
#!/usr/bin/env python3
"""
aq - Agent Queue CLI
...
def send_message(channel: str, msg_type: str, content: str, 
    ...
    if not requests_dir.exists():
        ...
        print(f"Create it with: aq-channel create {channel}", file=sys.stderr)
        sys.exit(1)
...
def cmd_recv(args):
    ...
    print(f"_AQ_PROCESSING={processing_file}", file=sys.stderr)
...
def cmd_watch(args):
    ...
    while True:
        ...
        for msg_file in messages:
            ...
            try:
                env = os.environ.copy()
                env["AQ_MSG_ID"] = msg_id
                env["AQ_MSG_TYPE"] = msg["type"]
                env["AQ_MSG_FROM"] = msg["from"]["agent"]
                env["AQ_MSG_CONTENT"] = msg["content"]
                env["AQ_CHANNEL"] = args.channel
...
def main():
    parser = argparse.ArgumentParser(
        description="Agent Queue - file-based message bus for AI agents",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  aq send test-runner signal '{"signal": "tests_passed", "count": 42}'
  aq send app-agent command "deploy to staging" --wait
  aq recv my-channel --wait
  aq respond my-channel req_xxx --status success --result "done"
  aq watch my-channel --handler ./process.sh
  aq status
        """
    )
...
#+begin_src shell :tangle ~/.local/share/aq/handlers/echo.sh :shebang "#!/usr/bin/env bash"
#!/usr/bin/env bash
# Echo handler - just echoes back the message content
# Environment vars: AQ_MSG_ID, AQ_MSG_TYPE, AQ_MSG_FROM, AQ_MSG_CONTENT, AQ_CHANNEL

echo "Received: $AQ_MSG_CONTENT"
echo "From: $AQ_MSG_FROM"
echo "Type: $AQ_MSG_TYPE"
#+end_src

** Test Runner Signal Handler

#+begin_src shell :tangle ~/.local/share/aq/handlers/test-signal.sh :shebang "#!/usr/bin/env bash"
#!/usr/bin/env bash
...
case "$SIGNAL" in
    tests_passed)
        ...
        # Could trigger: aq send app-agent command "deploy staging"
        ;;
    tests_failed)
...
#+begin_src python :tangle ~/.local/share/aq/handlers/cc-bridge.py :shebang "#!/usr/bin/env python3"
#!/usr/bin/env python3
"""
Bridge handler for Claude Code agents.
Translates aq messages into Claude Code compatible format.
"""
...
def handle_status(msg: dict) -> tuple[str, str]:
    ...
    return "success", json.dumps({
        "status": "healthy",
        "agent": os.environ.get("AQ_MSG_FROM", "unknown"),
        "pid": os.getpid()
    })
...
#+begin_src ruby :tangle ~/.local/share/aq/lib/aq.rb
# frozen_string_literal: true

# Agent Queue Ruby client
# Usage:
#   require_relative 'aq'
#   AQ.send('my-channel', :signal, { signal: 'done', count: 42 })
#   msg = AQ.recv('my-channel', wait: true)
...
module AQ
  VERSION = '1.0.0'
  HOME = ENV.fetch('AQ_HOME', File.expand_path('~/.aq'))
  ...
  class << self
    ...
    def send(channel, type, content, reply_to: nil, ttl: 300)
      ...
      unless Dir.exist?(requests_dir)
        raise "Channel '#{channel}' does not exist. Create with: aq-channel create #{channel}"
      end
...
#+begin_src ruby :tangle ~/.local/share/aq/handlers/example.rb :shebang "#!/usr/bin/env ruby"
#!/usr/bin/env ruby
# Example Ruby handler for aq watch
...
aq send app-agent signal '{"signal": "tests_passed", "suite": "integration", "count": 147}'
#+end_src
...
aq watch app-agent --handler ~/.local/share/aq/handlers/test-signal.sh
#+end_src
...
aq-channel create my-agent
aq watch my-agent --handler ~/.local/share/aq/handlers/cc-bridge.py

# Terminal 2: Send commands
aq send my-agent command "analyze the error logs and summarize issues"
aq send my-agent eval "git status"
aq send my-agent status "{}" --wait
#+end_src
...
def aq_send(channel: str, msg_type: str, content: str, wait: bool = False) -> dict:
    cmd = ["aq", "send", channel, msg_type, content, "--json"]
    if wait:
...
#+begin_src conf :tangle ~/.config/systemd/user/aq-watcher@.service
[Unit]
...
[Service]
Type=simple
Environment=AQ_HOME=%h/.aq
ExecStart=%h/.local/bin/aq watch %i --handler %h/.local/share/aq/handlers/%i.sh
Restart=on-failure
RestartSec=5
...
#+begin_src shell
systemctl --user enable aq-watcher@my-agent
systemctl --user start aq-watcher@my-agent
#+end_src
...
#+begin_src shell
# Tangle everything
emacs --batch -l org --eval '(org-babel-tangle-file "agent-bus-spec.org")'

# Make executables
chmod +x ~/.local/bin/aq*
chmod +x ~/.local/share/aq/handlers/*.sh
chmod +x ~/.local/share/aq/handlers/*.py
chmod +x ~/.local/share/aq/handlers/*.rb
...
| Command                    | Description                      |
|----------------------------+----------------------------------|
| =aq-init=                  | Initialize ~/.aq                 |
| =aq-channel create NAME=   | Create a channel                 |
| =aq send CHAN TYPE CONTENT= | Send message                    |
| =aq recv CHAN [--wait]=    | Receive one message              |
| =aq respond CHAN ID=       | Send response                    |
| =aq watch CHAN -H SCRIPT=  | Watch and handle messages        |
| =aq status=                | Show bus status                  |
| =aq ls CHAN [SUBDIR]=      | List messages                    |

** Environment Variables

| Variable    | Default     | Description              |
|-------------+-------------+--------------------------|
| =AQ_HOME=   | =~/.aq=     | Agent bus home directory |
| =AQ_MSG_ID= | (set by aq) | Current message ID       |
| =AQ_MSG_TYPE= | (set by aq) | Current message type   |
| =AQ_MSG_FROM= | (set by aq) | Sender agent address   |
| =AQ_MSG_CONTENT= | (set by aq) | Message content     |
| =AQ_CHANNEL= | (set by aq) | Current channel         |
#+end_src
