...
A minimal file-based message bus for coordinating AI agents across repositories, worktrees, and processes. Inspired by [Efrit's queue system](https://github.com/steveyegge/efrit) but generalized for any agent-to-agent communication.
...
See [`agent-bus-spec.org`](agent-bus-spec.org) for the full tangleable implementation in Org mode.
...
[Efrit](https://github.com/steveyegge/efrit) pioneered the file-based queue approach for AI agent communication:
...
Efrit uses JSON messages with types: `eval`, `command`, `chat`, `status`. Other agents (Claude Code, Cursor) can interact via the [Model Context Protocol](https://modelcontextprotocol.io/) or directly via the filesystem queue.

### 2. Unix Spool Pattern

The [spool directory pattern](https://en.wikipedia.org/wiki/Spooling) is a venerable Unix approach:
...
The [Maildir](https://en.wikipedia.org/wiki/Maildir) format (used by qmail, Dovecot) solves similar problems:
...
The [Actor Model](https://en.wikipedia.org/wiki/Actor_model) (Hewitt, 1973) defines agents with:
...
From [Erlang OTP](https://www.erlang.org/doc/design_principles/des_princ.html):
...
From [IPC literature](https://en.wikipedia.org/wiki/Inter-process_communication):
...
[FIPA Contract Net](http://www.fipa.org/specs/fipa00029/SC00029H.html) (1980) formalized multi-agent task allocation:
...
[Pub/Sub](https://en.wikipedia.org/wiki/Publish%E2%80%93subscribe_pattern) decouples producers from consumers:
...
| Option | Verdict |
|--------|---------|
| **Named FIFO** | Tempting, zero deps, but unidirectional and no persistence. If receiver isn't listening when you write, message is gone. |
...
| **Efrit/fs-queue** | Debuggable with `cat` and `jq`. Survives restarts. Works from any language. No daemon. Polling at 100ms is fine for "tests finished". |
...
| Multiple machines | NATS (single binary, trivial setup) |
...
| **Unix sockets** | ~0.1ms | Fast, stream-based | Same machine, high-volume |
| [**NATS**](https://nats.io/) | ~0.5ms | Pub/sub, req/reply | Multi-machine |
| [**ZeroMQ**](https://zeromq.org/) | ~0.1ms | No broker, patterns | Library integration |
| [**Redis**](https://redis.io/) | ~0.5ms | Pub/sub, persistence | Already running Redis |
...
[fswatch](https://github.com/emcrisostomo/fswatch) provides cross-platform file watching:
```bash
...
### Papers
- [A Communication Model to Integrate Request-Response and Publish-Subscribe](https://pmc.ncbi.nlm.nih.gov/articles/PMC3435995/) (PMC, 2012)
- [Survey of Agent Interoperability Protocols: MCP, ACP, A2A, ANP](https://arxiv.org/html/2505.02279v1) (ArXiv, 2025)
- [Ripple Effect Protocol: Coordinating Agent Populations](https://iceberg.mit.edu/protocol.pdf) (MIT, 2024)

### Specifications
- [FIPA Contract Net Interaction Protocol](http://www.fipa.org/specs/fipa00029/SC00029H.html)
- [Model Context Protocol](https://modelcontextprotocol.io/)
- [Agent Network Protocol](https://agent-network-protocol.com/specs/white-paper.html)

### Implementations
- [Efrit](https://github.com/steveyegge/efrit) - Emacs AI agent with file-based queue
- [fswatch](https://github.com/emcrisostomo/fswatch) - Cross-platform file change monitor
- [Maildir](https://cr.yp.to/proto/maildir.html) - djb's maildir specification
