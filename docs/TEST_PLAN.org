...
*** DONE FreeBSD 14 -> FreeBSD 15 (via VPN + SSH)

Validated 2026-01-28: cross-machine messaging between two FreeBSD hosts
connected via VPN, using SSH+SCP as the transport layer.

**** Test Environment

| Role | OS | Watcher Backend | inotify |
|----------+--------------------+-----------------+---------|
| host-a | FreeBSD 15.x | inotify (pyinotify) | Yes (native) |
| host-b | FreeBSD 14.x | polling | No |

Configure =$REMOTE_HOST= in =.env= (see =.env.template=).

**** Procedure

#+begin_src bash
# 1. Setup: ensure SSH key auth (one-time)
ssh-copy-id $REMOTE_HOST

# 2. On host-a: init abq and create receiving channel
uv run abq init
uv run abq channel create reports

# 3. On host-b (via SSH): pull latest, init abq, send message
ssh $REMOTE_HOST 'cd /path/to/abq && git pull && uv sync'
ssh $REMOTE_HOST 'cd /path/to/abq && uv run abq init && \
  uv run abq send reports signal \
    "{\"signal\": \"test_complete\", \"pyinotify\": true, \
      \"version\": \"15.0-RELEASE\", \"from_host\": \"host-b\"}"'

# 4. Sync messages from host-b to host-a (rsync is idempotent)
rsync -avz $REMOTE_HOST:~/.abq/channels/ ~/.abq/channels/

# 5. On host-a: receive and verify
...
Content: {"signal": "test_complete", "pyinotify": true, "version": "15.0-RELEASE", "from_host": "host-b"}
#+end_example
...
- VPN provides stable addressing across networks
